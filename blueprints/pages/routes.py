...
@pages_bp.route('/')
def index():
    """Landing page - redirect to login if not logged in, dashboard if logged in"""
...
@login_required
def catalog():
    """Portfolios gallery for logged-in users"""
    data = load_data()
    ...
    for username, port in portfolios.items():
        ...
        port['is_verified'] = is_verified
        port['is_demo'] = is_demo
        
        if is_verified:
            verified_portfolios.append(port)
        elif is_demo:
            demo_portfolios.append(port)
        else:
            full_portfolios.append(port)
    
    return render_template('catalog.html', 
                           verified_portfolios=verified_portfolios,
                           demo_portfolios=demo_portfolios,
                           full_portfolios=full_portfolios)
