...
from utils.data import load_data, save_data, get_or_create_workspace
...
from utils.notifications import send_telegram_notification, get_telegram_credentials, load_smtp_config, send_event_notification_async, send_email, get_admin_notifications_config
from utils.badges import determine_badge
from models import User, db, Message, Workspace, NotificationSettings
    ...
    if is_admin:
        ...
        if user:
            ...
            if not workspace_id:
                ...
                # Count user's data
                # "Active Projects" should reflect portfolio/training projects (exclude request entries)
                user_projects = Project.query.filter(
                    Project.workspace_id == workspace_id,
                    Project.project_type != 'request'
                ).count()
                ...
                # "Service Offerings" should reflect active services
                user_services = Service.query.filter(
                    Service.workspace_id == workspace_id,
                    Service.is_active == True
                ).count()
                # "Inbound Requests" should reflect public portfolio inquiries only (exclude internal chat)
                user_messages = Message.query.filter(
                    Message.workspace_id == workspace_id,
                    Message.is_internal == False,
                    Message.category == 'portfolio'
                ).count()
                user_unread = Message.query.filter(
                    Message.workspace_id == workspace_id,
                    Message.is_internal == False,
                    Message.category == 'portfolio',
                ...
                # Visitors (count by UTC day range for reliability across DBs)
                start_of_today = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
                start_of_tomorrow = start_of_today + timedelta(days=1)
                today_visitors = VisitorLog.query.filter(
                    VisitorLog.workspace_id == workspace_id,
                    VisitorLog.created_at >= start_of_today,
                    VisitorLog.created_at < start_of_tomorrow
                ).count()
...
def general():
    ...
    if request.method == 'POST':
        ...
        try:
            user = User.query.filter_by(username=username).first()
            workspace = (db.session.get(Workspace, user.workspace_id) if user and user.workspace_id else None)
            workspace = workspace or get_or_create_workspace(username, data.get('name'))
            if user and not user.workspace_id:
            ...
            workspace.photo = data.get('photo', '')
            db.session.add(workspace)
            db.session.commit()
            flash('General information saved successfully', 'success')
        except Exception as e:
            current_app.logger.error(f"General save error for {username}: {str(e)}")
            db.session.rollback()
            # Fallback to legacy save_data behavior
            save_data(data, username=username)
            flash('General information saved with fallback. Please verify your changes.', 'warning')
...
def about():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            workspace.about = data.get('about', '')
            db.session.add(workspace)
            db.session.commit()
            flash('About section saved successfully', 'success')
        except Exception as e:
            current_app.logger.error(f"About save error for {username}: {str(e)}")
            db.session.rollback()
            save_data(data, username=username)
            flash('About section saved with fallback. Please verify your changes.', 'warning')
...
def skills():
    ...
    if request.method == 'POST':
        ...
        # Persist to the user's workspace_id to avoid writing to a different workspace
        user = User.query.filter_by(username=username).first()
        workspace_id = user.workspace_id if user else None
        if not workspace_id:
            current_app.logger.warning(f"Skills save: user {username} has no workspace_id; falling back to save_data")
            data['skills'] = skills
            save_data(data, username=username)
        else:
            from models import Skill
            Skill.query.filter_by(workspace_id=workspace_id).delete()
...
def add_project():
    ...
    if request.method == 'POST':
        ...
        if not title:
            ...
            return redirect(url_for('dashboard.add_project'))
...
def edit_project(project_id):
    ...
    if request.method == 'POST':
        ...
        if not title:
            flash('Project title is required', 'error')
            return redirect(url_for('dashboard.edit_project', project_id=project_id))
...
def contact():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            workspace.contact = data.get('contact', {})
            db.session.add(workspace)
            db.session.commit()
            flash('Contact information saved successfully', 'success')
        except Exception as e:
            current_app.logger.error(f"Contact save error for {username}: {str(e)}")
            db.session.rollback()
            save_data(data, username=username)
            flash('Contact information saved with fallback. Please verify your changes.', 'warning')
...
def social():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            workspace.social = data.get('social', {})
            db.session.add(workspace)
            db.session.commit()
            flash('Social links saved successfully', 'success')
        except Exception as e:
            current_app.logger.error(f"Social save error for {username}: {str(e)}")
            db.session.rollback()
            save_data(data, username=username)
            flash('Social links saved with fallback. Please verify your changes.', 'warning')
...
def settings():
    ...
    if request.method == 'POST':
        ...
        if selected_theme in valid_themes:
            # For user dashboard: persist theme directly to the user's linked workspace
            # to avoid DB mix-ups and rollback/fallback behaviors inside save_data.
            if not is_admin:
                try:
                    user = User.query.filter_by(username=username).first()
                    workspace = (db.session.get(Workspace, user.workspace_id) if user and user.workspace_id else None)
                    workspace = workspace or get_or_create_workspace(username)
                    if user and not user.workspace_id:
                        user.workspace_id = workspace.id
                        db.session.add(user)
                    ...
                    current_app.logger.error(f"Theme save error for {username}: {str(e)}")
                    db.session.rollback()
                    flash('Failed to save theme. Please try again.', 'error')
            else:
                data['settings']['theme'] = selected_theme
                save_data(data, username=username)
                flash(f'Theme changed to {selected_theme.replace("-", " ").title()} successfully', 'success')
    ...
    if is_admin:
        admin_config = get_admin_notifications_config()
        bot_token = admin_config['telegram'].get('bot_token')
        chat_id = admin_config['telegram'].get('chat_id')
        smtp_cfg = admin_config['smtp']
    else:
...
def telegram_settings():
    ...
    if session.get('is_admin'):
        flash('Admin notifications are managed via environment variables (.env).', 'info')
        return redirect(url_for('dashboard.settings'))
    ...
    try:
        ...
        # Save to database notification settings
        user = User.query.filter_by(username=username).first()
        workspace = (db.session.get(Workspace, user.workspace_id) if user and user.workspace_id else None)
        workspace = workspace or get_or_create_workspace(username)
        if workspace:
            ...
            notif_settings.telegram_bot_token = bot_token
            notif_settings.telegram_chat_id = chat_id
            notif_settings.telegram_configured_at = datetime.now()
            db.session.commit()
...
def smtp_settings():
    ...
    username = session.get('username')
    if session.get('is_admin'):
        flash('Admin SMTP settings are managed via environment variables (.env).', 'info')
        return redirect(url_for('dashboard.settings'))
...
def add_client():
    ...
    if request.method == 'POST':
        ...
        if not client_name:
            ...
            return redirect(url_for('dashboard.add_client'))
...
def edit_client(client_id):
    ...
    if request.method == 'POST':
        ...
        if not client_name:
            flash('Client name is required', 'error')
            return redirect(url_for('dashboard.edit_client', client_id=client_id))
