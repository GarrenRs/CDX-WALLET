...
from models import Workspace, NotificationSettings
        ...
        try:
            ...
            if workspace:
                ...
                if notif_settings and notif_settings.smtp_config:
                    smtp_cfg = notif_settings.smtp_config
                    ...
                    if all([
                        ...
                        current_app.logger.debug(f"Loaded user SMTP config from DB for {username}")
                        return smtp_cfg
        except Exception as e:
            current_app.logger.debug(f"Could not load user SMTP config from DB for {username}: {str(e)}")

        # Fallback to legacy JSON-loaded data
        try:
            ...
            if 'notifications' in user_data and 'smtp' in user_data['notifications']:
                ...
                if all([
                    ...
                    current_app.logger.debug(f"Loaded user-specific SMTP config for {username} from legacy data")
                    return smtp_cfg
        except Exception as e:
...
def get_telegram_credentials(username=None):
    ...
    """
    if username:
        # Get user-specific credentials from database first
        try:
            workspace = Workspace.query.filter_by(slug=username).first()
            if workspace:
                notif_settings = NotificationSettings.query.filter_by(workspace_id=workspace.id).first()
                if notif_settings and notif_settings.telegram_bot_token and notif_settings.telegram_chat_id:
                    ...
                    if (bot_token == admin_config.get('bot_token') and 
                        ...
                        return None, None
                    current_app.logger.debug(f"Loaded user-specific Telegram credentials from DB for {username}")
                    return bot_token, chat_id
        except Exception as e:
            current_app.logger.debug(f"Could not load Telegram credentials from DB for {username}: {str(e)}")
            ...
            if 'notifications' in user_data and 'telegram' in user_data['notifications']:
                ...
                if bot_token and chat_id:
                    ...
                    if (bot_token == admin_config.get('bot_token') and 
                        ...
                        current_app.logger.warning(f"User {username} Telegram config matches admin config - ignoring to prevent mix-up")
                        return None, None
                    current_app.logger.debug(f"Loaded user-specific Telegram credentials for {username} from legacy data")
                    return bot_token, chat_id
        except Exception as e:
