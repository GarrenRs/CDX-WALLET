...
def load_data(username=None):
    ...
    try:
        ...
        if username:
            # Prefer the user's linked workspace_id to avoid mixing workspaces
            ...
            # Fallback to slug-based workspace lookup
            if not workspace:
                workspace = get_workspace_by_username(username)
...
def workspace_to_dict(workspace):
    ...
    if notif_settings and notif_settings.smtp_config:
        notifications['smtp'] = notif_settings.smtp_config
...
def save_data(user_data, username=None, auto_backup=True):
    ...
    try:
        ...
        if not workspace:
            ...
            # If a DB user exists but isn't linked properly, repair linkage
        ...
        # Update notification settings (Telegram/SMTP) for this workspace
        notifications = user_data.get('notifications', {})
        if notifications:
            telegram_cfg = notifications.get('telegram', {})
            smtp_cfg = notifications.get('smtp', {})
            if telegram_cfg or smtp_cfg:
                notif_settings = NotificationSettings.query.filter_by(workspace_id=workspace.id).first()
                ...
                if telegram_cfg:
                    notif_settings.telegram_bot_token = telegram_cfg.get('bot_token') or None
                    notif_settings.telegram_chat_id = telegram_cfg.get('chat_id') or None
                    configured_at = telegram_cfg.get('configured_at')
                    if configured_at:
                        try:
                            notif_settings.telegram_configured_at = datetime.strptime(
                                configured_at, '%Y-%m-%d %H:%M:%S'
                            )
                        except ValueError:
                            notif_settings.telegram_configured_at = datetime.utcnow()
                    elif notif_settings.telegram_bot_token and notif_settings.telegram_chat_id:
                        notif_settings.telegram_configured_at = datetime.utcnow()

                if smtp_cfg:
                    notif_settings.smtp_config = smtp_cfg
        ...
        if 'skills' in user_data:
            ...
            for skill_data in user_data.get('skills', []):
                name_raw = (skill_data.get('name') or '').strip()
                if not name_raw:
                    continue
                try:
                    level_val = int(skill_data.get('level', 50))
        ...
        if 'projects' in user_data:
            ...
            for project_data in user_data.get('projects', []):
                title_raw = (project_data.get('title') or '').strip()
                if not title_raw:
                    # title is required by the model; skip invalid legacy items
                    continue
                ...
                project = Project(
                    workspace_id=workspace.id,
                    id=project_id,  # Ensure a valid PK exists
                    title=title_raw,
        ...
        if 'services' in user_data:
            ...
            for service_data in user_data.get('services', []):
                title_raw = (service_data.get('title') or '').strip()
                if not title_raw:
                    # title is required by the model; skip invalid legacy items
                    continue
                ...
                service = Service(
                    workspace_id=workspace.id,
                    id=service_id,  # Ensure a valid PK exists
                    title=title_raw,
        ...
        if 'clients' in user_data:
            ...
            for client_data in user_data.get('clients', []):
                name_raw = (client_data.get('name') or '').strip()
                if not name_raw:
                    # name is required by the model; skip invalid legacy items
                    continue
                ...
                client_id = client_data.get('id') or str(uuid.uuid4())
                
                client = Client(
                    workspace_id=workspace.id,
                    id=client_id,  # Ensure a valid PK exists
                    name=name_raw,
...
def get_global_meta():
    ...
    return {
        'title': 'Codexx Wallets | Elite Proof-of-Work Ecosystem',
        'description': 'The premier ecosystem for verified professionals. Build in silence, show in public.',
        'keywords': 'Codexx Wallets, Proof of Work, Elite Professionals, Portfolio Ecosystem'
    }
